...
local function CreateESP(player)
    ...
    espData.Update = function(deltaTime)
        ...
        if CONFIG.ESP.ShowWeapon then
            ...
            espData.Drawings.Weapon.Text = tool and tool.Name or "None"
...
local function GetTarget()
    ...
    if mode == "FOV" then
        ...
        for _, t in ipairs(validTargets) do
            ...
            if t.Distance2D < bestDist then
                bestDist = t.Distance2D
                best = t
            end
        end
    ...
    if mode == "Nearest" then
        ...
        for _, t in ipairs(validTargets) do
            if t.Distance3D < bestDist then
                bestDist = t.Distance3D
                best = t
            end
        end
...
local function StartOrbit()
    ...
    OrbitConnection = RunService.RenderStepped:Connect(function()
        ...
        if CONFIG.Player.OrbitTarget then
        ...
        if targetPos then
            ...
            local height = math.sin(time * 0.5) * dist * 0.3
...
ESPTab:CreateDropdown({
    ...
    Options = {"Bottom", "Top", "Left", "Right", "Center", "Mouse"},
...
local orbitDropdown = PlayersTab:CreateDropdown({
    Name = "Orbit Target",
    Options = {"None"},
    CurrentOption = "None",
    Flag = "OrbitTarget",
    Callback = function(v)
        CONFIG.Player.OrbitTarget = v == "None" and nil or v
    end
})

PlayersTab:CreateButton({
    Name = "🔄 Refresh Orbit List",
    Callback = function()
        local names = {"None"}
        for _, p in ipairs(Players:GetPlayers()) do
...
RunService.RenderStepped:Connect(function()
    ...
    -- Auto apply WalkSpeed/JumpHeight
    local char = GetCharacter(LocalPlayer)
    if char then
        local humanoid = char:FindFirstChildOfClass("Humanoid")
        if humanoid then
...
task.spawn(function()
    ...
    targetDropdown:Refresh(names)
    allyDropdown:Refresh(names)
    orbitDropdown:Refresh({"None", table.unpack(names)})
