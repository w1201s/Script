...
local function CreateESP(player)
    ...
    espData.Update = function(deltaTime)
        ...
        if CONFIG.ESP.UseRGB then
            displayColor = Color3.fromHSV(SharedRGBHue, 1, 1)
        else
            if isTargeted then
...
local Window = WindUI:CreateWindow({
    ...
    KeySystem = {                                                   
        Note = "Do the keysystem now for op script trust discord link soon",              
        API = {                                                     
            { -- PlatoBoost
                Type = "platoboost",                                
...
-- FIXED: Added "Center" option to tracer position
ESPTab:Dropdown({
...
-- JumpPower with Toggle - FIXED VERSION
PlayersTab:Toggle({
...
--// COLORS TAB
ColorsTab:Section({Title = "RGB & Colors", Opened = true})

ColorsTab:Toggle({
    Title = "Shared RGB Mode",
    Desc = "Rainbow cycling for both ESP and FOV",
...
RunService.RenderStepped:Connect(function(deltaTime)
    ...
    if CONFIG.ESP.UseRGB then
        SharedRGBHue = (SharedRGBHue + deltaTime * CONFIG.ESP.RGBSpeed * 0.1) % 1
        FOVCircle.Color = Color3.fromHSV(SharedRGBHue, 1, 1)
    else
...
print("✅ Venus x Loaded with WindUI!")
